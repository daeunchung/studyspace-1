...
// 다른 때에도 이 함수를 사용해 주기 위해서 독립시켜서 선언해줌
sNode * find_snode(Artist * ptr_artist, char * title) {
...
void insert_to_index_directory(Song * ptr_song) {
	...
	sNode * q = NULL;	// ordered single linked list 에 삽입하기 위해서는 포인터 하나 더 필요함
...
Artist * find_artist(char * name) {
	...
	// 0 ~ 256  사이값으로 해주기 위해서 unsigned char 로 ! 
	...
	/* 1101111 -> 111111111111111111111001111 실제로는 이렇게 돼서 음수처리 된다 => unsigned 로 명시필요 */
...
void play(int index) {
	...
	// 내 윈도우 플레이어에서 어떻게 하면 가장 간단하게 음악을 플레이 하는가
	// 파일을 더블클릭해서 실행하는 것
...
void remove(int index) {
	...
	if (q == NULL) {		// remove first
		...
		q->next = p->next;	// q->next = q->next->next; 와 같은소리
	}
	free(p);
